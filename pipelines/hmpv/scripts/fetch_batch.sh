...
#   # ...then:  cd ..;  CONFIG=config_hmpv.yaml sbatch run_hmpv.sh
#
...
SAMPLE_DIR="${SAMPLE_DIR:-/home/jk2666/project_pi_dmw63/jk2666/hmpv_yale}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PIPELINE_DIR="$(dirname "$SCRIPT_DIR")"
CONFIG_DIR="$PIPELINE_DIR/config"
CONFIG_TEMPLATE="${CONFIG_TEMPLATE:-$CONFIG_DIR/config_hmpv.yaml}"
...
[[ -n "$OUTCONFIG" ]] && {
    ...
    echo "    CONFIG=config_${TARGET}.yaml sbatch run_hmpv.sh"
}
if [[ "$bad" -eq 0 ]]; then
