...
#age color scheme
age_colors         <- rev(brewer.pal(5,"RdYlGn"))
#age_colors         <- c("#008837","#A6DBA0","#F7F7F7","#C2A5CF","#7B3294")
names(age_colors)  <- c("<1",     "[1,5)",  "[5,18)", "[18,65)","65+")
...
# make graph from network with positions based on tree --------------------



get_network_tree_df <- function(transmatrix,tree,min_z=0.5) {
  treetab <- subset(fortify(tree),isTip)
  setmatrixdf <- reshape2::melt(transmatrix, c("from", "to"), 
                                value.name = "z") %>% filter(z>=min_z) %>%
    merge(treetab[,c("label","x","y")],by.x="to",by.y="label",all.x=T) %>%
    merge(treetab[,c("label","x","y")],by.x="from",by.y="label",all.x=T,
          suffixes = c("_to","_from"))
  return(setmatrixdf)
}
